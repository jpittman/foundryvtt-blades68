...
Hooks.once("init", async function() {
  ...
  Handlebars.registerHelper('oneless', (a) => {
    ...
    for (let i = 1; i <= max_rep; i++) {
      ...
      if (i > max_rep - turfs_amount_int) {
		  html += `<input disabled type="radio" id="crew-${_id}-reputation-${i}" name="system.reputation" value="${i} dtype="Radio"><label style="background-image: url('systems/blades68/styles/assets/teeth/stresstooth-black.png')" class="radio-toggle" for="crew-${_id}-reputation-${i}"></label>`;
	  } else {
  ...
  Handlebars.registerHelper('blades-clock-color', function(parameter_name, type, color, current_value, uniq_id) {
    ...
    html += `<div id="blades-clock-${uniq_id}" class="blades-clock clock-${type} clock-${type}-${current_value}" style="background-image:url('systems/blades68/themes/${color}/${type}clock_${current_value}.svg');">`;
  ...
  Handlebars.registerHelper('blades-clock', function(parameter_name, type, current_value, uniq_id) {
    ...
    // Label for 0
    html += `<label class="clock-zero-label" for="clock-0-${uniq_id}}"><i class="fab fa-creative-commons-zero nullifier"></i></label>`;
    html += `<div id="blades-clock-${uniq_id}" class="blades-clock clock-${type} clock-${type}-${current_value}" style="background-image:url('systems/blades68/themes/black/${type}clock_${current_value}.svg');">`;
